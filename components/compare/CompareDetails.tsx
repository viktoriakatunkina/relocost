...
import type { CompareFaqItem } from "@/lib/compare";
import { typo } from "@/lib/typography";

// Текст-вывод + FAQ для compare-страницы. Контент генерируется из чисел
// сравнения (lib/compare.ts), здесь — только отрисовка + FAQPage JSON-LD.
        ...
        <section className="max-w-4xl mx-auto px-6 pt-14 md:pt-20">
          ...
          <div className="space-y-4">
            {summary.map((p, i) => (
              <p
                key={i}
                className="text-brandy/90 text-lg leading-relaxed text-pretty"
              >
                {typo(p)}
              </p>
            ))}
              ...
              return (
                <div
                  ...
                  className={`rounded-3xl border overflow-hidden transition ${
                    isOpen
                      ? "bg-surface-elevated border-copper/30"
                      : "bg-surface hairline hover:border-copper/20"
                  }`}
                >
                  <button
                    type="button"
